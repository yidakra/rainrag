...
def test_ingest_command_success():
    ...
    with patch('src.rainrag.cli.run_ingestion') as mock_ingest:
        with patch('src.rainrag.cli.load_config'):
            ...
            assert "Starting ingestion pipeline" in result.stdout
            assert "Ingestion complete" in result.stdout
            assert "100 documents" in result.stdout
...
def test_embed_command_success():
    ...
    with patch('src.rainrag.cli.run_embedding') as mock_embed:
        with patch('src.rainrag.cli.load_config'):
            ...
            assert result.exit_code == 0
            assert "Starting embedding generation" in result.stdout
            assert "Embedding complete" in result.stdout
            assert "10 documents" in result.stdout
...
def test_embed_command_failure():
    ...
    with patch('src.rainrag.cli.run_embedding') as mock_embed:
        with patch('src.rainrag.cli.load_config'):
            ...
            assert result.exit_code == 1
            assert "Embedding failed" in result.stdout
...
def test_index_command_success():
    ...
    with patch('src.rainrag.cli.run_indexing') as mock_index:
        with patch('src.rainrag.cli.load_config'):
            ...
            assert result.exit_code == 0
            assert "Starting indexing pipeline" in result.stdout
...
def test_index_command_with_recreate_option():
    ...
    with patch('src.rainrag.cli.run_indexing') as mock_index:
        with patch('src.rainrag.cli.load_config'):
            ...
            assert result.exit_code == 0
            assert "Recreating collection" in result.stdout
        ...
        with patch('src.rainrag.cli.run_embedding') as mock_embed:
            with patch('src.rainrag.cli.run_indexing') as mock_index:
                with patch('src.rainrag.cli.load_config'):
                    ...
                    assert result.exit_code == 0
                    assert "Starting full pipeline" in result.stdout
                    assert "Step 1/3: Ingestion" in result.stdout
                    assert "Step 2/3: Embedding" in result.stdout
                    assert "Step 3/3: Indexing" in result.stdout
                    assert "Full pipeline complete" in result.stdout
        ...
        with patch('src.rainrag.cli.run_embedding') as mock_embed:
            with patch('src.rainrag.cli.run_indexing') as mock_index:
                with patch('src.rainrag.cli.load_config'):
                    ...
                    assert result.exit_code == 0
                    assert "Ingestion (skipped)" in result.stdout
        ...
        with patch('src.rainrag.cli.run_embedding') as mock_embed:
            with patch('src.rainrag.cli.run_indexing') as mock_index:
                with patch('src.rainrag.cli.load_config'):
                    ...
                    assert result.exit_code == 0
                    assert "Embedding (skipped)" in result.stdout
...
def test_ask_command_success():
    ...
    with patch('src.rainrag.cli.run_query') as mock_query:
        with patch('src.rainrag.cli.load_config'):
            ...
            assert "Retrieved 3 relevant documents" in result.stdout
...
def test_ask_command_with_verbose():
    ...
    with patch('src.rainrag.cli.run_query') as mock_query:
        with patch('src.rainrag.cli.load_config'):
            ...
            assert "/test/doc1.vtt" in result.stdout
...
def test_info_command_success():
    ...
    with patch('src.rainrag.cli.load_config') as mock_load_config:
        with patch('src.rainrag.cli.QdrantIndexer') as mock_indexer:
            ...
            assert result.exit_code == 0
            assert "RainRAG Configuration" in result.stdout
            assert "Archive root" in result.stdout
            assert "/test/archive" in result.stdout
...
def test_main_help():
    ...
    assert result.exit_code == 0
    assert "rainrag" in result.stdout.lower()
...
def test_ingest_help():
    ...
    assert result.exit_code == 0
    assert "Ingest and parse VTT files" in result.stdout
